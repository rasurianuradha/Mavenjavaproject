...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="memorypool">
		<table>
			<tr>
				...
				<td>
					<b>Initial:</b>
					<xsl:value-of select="@usageInit" />
				</td>
				<td>
					<b>Committed:</b>
					<xsl:value-of select="@usageCommitted" />
				</td>
				<td>
					<b>Maximum:</b>
					<xsl:value-of select="@usageMax" />
				</td>
				<td>
					<b>Used:</b>
					<xsl:value-of select="@usageUsed" />
				</td>
			</tr>
		</table>
	</xsl:template>
	...
	<xsl:template match="threadInfo">
		<table>
			<tr>
				...
				<td>
					<b>currentThreadCount:</b>
					<xsl:value-of select="@currentThreadCount" />
				</td>
				<td>
					<b>currentThreadsBusy:</b>
					<xsl:value-of select="@currentThreadsBusy" />
				</td>
			</tr>
		</table>
		<hr />
	...
	<xsl:template match="worker">
		<tr>
			...
			<td>
				<xsl:value-of select="@requestProcessingTime" />
			</td>
			<td>
				<xsl:value-of select="@requestBytesSent" />
			</td>
			<td>
				<xsl:value-of select="@requestBytesReceived" />
			</td>
			<td>
			...
			<td><xsl:value-of select="@method" /> <xsl:value-of
					...
					select="@currentQueryString" /> <xsl:value-of
					select="@protocol" /></td>
		</tr>
	</xsl:template>
